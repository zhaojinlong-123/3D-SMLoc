...
public:
    ...
    {
        pose.fromVector(v.head<7>()); //掉用g2o库
        scale = v.tail<3>();
    }
    ...
    //apply update to current cuboid
    ...
    Vector9d min_log_error(const cuboid &newone, bool print_details = false) const
    {
        ...
        for (int i = 0; i < rotate_errors_norm.rows(); i++)
        {
            ...
            rotate_errors_norm(i) = cuboid_error.norm(); //均方根
    ...
    cuboid rotate_cuboid(double yaw_angle) const
    {
        cuboid res;
    ...
    {
        cuboid res;
        res.pose = Twc*this->pose; //*Twc.inverse();
        res.scale = this->scale;
        return res;
    }
	...
	{
		cuboid res;
		res.pose = Twc.inverse() * this->pose; //*Twc;
		res.scale = this->scale;
		return res;
	}
    ...
    {
        Matrix4d res = pose.to_homogeneous_matrix(); //齐次矩阵
        Matrix3d scale_mat = scale.asDiagonal(); //对角矩阵
