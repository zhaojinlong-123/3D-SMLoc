...
int max_index(int a, int b, int c, int d, int e){
    ...
    num[3] = d;
    num[4] = e;
    int max = num[0]; //此处可以改变权重，代表背景数量的重要程度，越不重要，倍数越大
    int index = 0;
    for(int i = 1; i < 5; i++){
        if(num[i] >= max){
            max = num[i];
...
MatrixXd SCManager::makeScancontext( pcl::PointCloud<SCPointType> & _scan_down )
{
    ...
    MatrixXd temp = NO_POINT * MatrixXd::Ones(PC_NUM_RING, 5*PC_NUM_SECTOR);

    SCPointType pt;
    float azim_angle, azim_range; // wihtin 2d plane
    int ring_idx, sctor_idx;

    // 扫描每一个点，统计其对应区块中标签的数量（包含背景）
    for (int pt_idx = 0; pt_idx < num_pts_scan_down; pt_idx++)
    {
        pt.x = _scan_down.points[pt_idx].x; 
        pt.y = _scan_down.points[pt_idx].y;
        pt.z = _scan_down.points[pt_idx].z + LIDAR_HEIGHT; // naive adding is ok (all points should be > 0).
        pt.r = _scan_down.points[pt_idx].r; 
        pt.g = _scan_down.points[pt_idx].g; 
        pt.b = _scan_down.points[pt_idx].b; 
        ...
        int ring = ring_idx - 1;
        int sctor = sctor_idx - 1;

        if( pt.r == 0 && pt.g == 0 && pt.b == 0 )
            temp(ring,5*sctor)++;
        if( pt.r == 0 && pt.g == 0 && pt.b == 128 )
            temp(ring,5*sctor+1)++;
        if( pt.r == 0 && pt.g == 128 && pt.b == 0 )
            temp(ring,5*sctor+2)++;
        if( pt.r == 128 && pt.g == 0 && pt.b == 0 )
            temp(ring,5*sctor+3)++;
        if( pt.r == 128 && pt.g == 128 && pt.b == 0 )
            temp(ring,5*sctor+4)++;
    }

    // label 取 0-4，分别对应背景、减速带、车道线、箭头、停车位线
    for ( int row_idx = 0; row_idx < temp.rows(); row_idx++ ){
        for ( int col_idx = 0; col_idx < temp.cols(); col_idx =  col_idx + 5){
            int label = max_index( temp(row_idx,col_idx), temp(row_idx,col_idx+1), temp(row_idx,col_idx+2), temp(row_idx,col_idx+3), temp(row_idx,col_idx+4) );
            desc(row_idx, col_idx/5) = label; 
        }
    }
...
std::pair<int, float> SCManager::detectLoopClosureIDBetweenSession (std::vector<float>& _curr_key, Eigen::MatrixXd& _curr_desc)
{
    ...
    if( ! is_tree_batch_made ) // run only once
    {
        ...
        polarcontext_tree_batch_ = std::unique_ptr<InvKeyTree>(new InvKeyTree(PC_NUM_RING /* dim */, polarcontext_invkeys_to_search_, 10 /* max leaf */ ));
