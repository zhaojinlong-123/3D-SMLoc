...
std::string mapSaveLocation="/home/zjl/avpslam_ws/src/AVP-SLAM-PLUS/avp_slam_plus/data/PGOCloudMap.pcd";
std::string save_directory = "/home/zjl/avpslam_ws/src/AVP-SLAM-PLUS/avp_slam_plus/data";
...
void pubPath( void )
{
    ...
    for (int node_idx=0; node_idx < recentIdxUpdated; node_idx++) // -1 is just delayed visualization (because sometimes mutexed while adding(push_back) a new one)
    {
        ...
        odomAftPGOthis.child_frame_id = "/camera0_link";
    ...
    br.sendTransform(tf::StampedTransform(transform, odomAftPGO.header.stamp, "/world", "/camera0_link"));
} // pubPath
...
void loopFindNearKeyframesCloud( pcl::PointCloud<PointType>::Ptr& nearKeyframes, const int& key, const int& submap_size, const int& root_idx)
{
    ...
    for (int i = -submap_size; i <= submap_size; ++i) {
        ...
        mKF.lock(); 
        *nearKeyframes += *local2global(keyframeLaserClouds[keyNear],keyframePosesUpdated.at(root_idx));
        mKF.unlock(); 
    }
...
std::optional<gtsam::Pose3> doICPVirtualRelative( int _loop_kf_idx, int _curr_kf_idx )
{
    ...
    cureKeyframeCloudMsg.header.frame_id = "/camera0_link";
    pubLoopScanLocal.publish(cureKeyframeCloudMsg);

    sensor_msgs::PointCloud2 targetKeyframeCloudMsg;
    pcl::toROSMsg(*targetKeyframeCloud, targetKeyframeCloudMsg);
    targetKeyframeCloudMsg.header.frame_id = "/camera0_link";
...
void pubMap(void)
{
    ...
    for (int node_idx=0; node_idx < recentIdxUpdated; node_idx++) {
        if(counter % SKIP_FRAMES == 0) {
            *laserCloudMapPGO += *local2global(keyframeLaserClouds[node_idx],keyframePosesUpdated.at(node_idx));
        }
        counter++;
    }
    mKF.unlock(); 
    ...
    pcl::io::savePCDFileASCII(save_directory+"PGOCloudMap.pcd", *laserCloudMapPGO);     
...
int main(int argc, char **argv)
{
    ...
    // save directories 
	nh.param<std::string>("save_directory", save_directory, "/home/zjl/avpslam_ws/src/AVP-SLAM-PLUS/avp_slam_plus/data/"); // pose assignment every k m move 
    ...
    // system params 
	nh.param<double>("keyframe_meter_gap", keyframeMeterGap, 0.3); // pose assignment every k m move 
	nh.param<double>("keyframe_deg_gap", keyframeDegGap, 5.0); // pose assignment every k deg rot 
    keyframeRadGap = deg2rad(keyframeDegGap);

	nh.param<double>("sc_dist_thres", scDistThres, 0.4);  
