...
public:
    ...
    void odomCallback(const nav_msgs::Odometry::ConstPtr &msg) {
        ...
        long double time_a = 1679113012.648966000; 
        if(abs(time_s-time_a)<eps){
            newTransWorldCurrent=true;
            //-25.279967650786148 264.1503442385167 0 0.020818280741544768 0.03420132796297845 0.7116828648063113 -0.7013589439749215
            //-24.997129334416755 265.0077918740741 0 -0.023994770901713744 -0.03358858895259227 -0.6379574746565476 0.7689644453363162
            //-25.195138046223686 265.1439738412794 0 -0.029168047843048387 -0.0397148533405955 -0.5975474252112694 0.8003180805358227
            transWorldCurrent.pose.orientation.w=0.8003180805358227;
            transWorldCurrent.pose.orientation.x=-0.029168047843048387;
            transWorldCurrent.pose.orientation.y=-0.0397148533405955;
            transWorldCurrent.pose.orientation.z=-0.5975474252112694;
            transWorldCurrent.pose.position.x=-25.195138046223686;
            transWorldCurrent.pose.position.y=265.1439738412794;
            transWorldCurrent.pose.position.z=0;
        }
        if(time_s<=1679113012.648966000){
            // 添加位姿到位姿图
            addPoseToGraph(msg);
            if(pose_ids_.size()>1){
                addEdgeBetweenPoses();
